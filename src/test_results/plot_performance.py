...
def plot_performance_lowest_level(data_folder):
    ...
    for (cycle, lowest_level), cycle_df in v_df.groupby(["CycleType", "LowestLevel"]):
        ...
        plt.plot(cycle_df["GridSize"], cycle_df["TotalTime"], marker='x', linestyle='--', label=f"{cycle}-cycle, L={lowest_level}, slope={slope:.2f}")
    
    for (cycle, lowest_level), cycle_df in w_df.groupby(["CycleType", "LowestLevel"]):
        slope, _, _, _, _ = linregress(np.log2(cycle_df["GridSize"]), np.log2(cycle_df["TotalTime"]))
        plt.plot(cycle_df["GridSize"], cycle_df["TotalTime"], marker='o', linestyle='-', label=f"{cycle}-cycle, L={lowest_level}, slope={slope:.2f}")

    # for (cycle, lowest_level), cycle_df in multigrid_df.groupby(["CycleType", "LowestLevel"]):
    #     if cycle == "V":
    #         slope, _, _, _, _ = linregress(np.log2(cycle_df["GridSize"]), np.log2(cycle_df["TotalTime"]))
    #         plt.plot(cycle_df["GridSize"], cycle_df["TotalTime"], marker='o', linestyle='--', label=f"{cycle}-cycle, L={lowest_level}, slope={slope:.2f}")
    #     if cycle == "W":
    #         slope, _, _, _, _ = linregress(np.log2(cycle_df["GridSize"]), np.log2(cycle_df["TotalTime"]))
    #         plt.plot(cycle_df["GridSize"], cycle_df["TotalTime"], marker='s', linestyle='-', label=f"{cycle}-cycle, L={lowest_level}, slope={slope:.2f}")
    ...
    plt.grid()
    plt.title("Cycle Depth Comparison", fontsize=16)
