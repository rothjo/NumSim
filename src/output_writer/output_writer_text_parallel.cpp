...
void OutputWriterTextParallel::writeFile(double currentTime)
{
  ...
  // write u values
  for (int j = discretization_->uJEnd(); j >= discretization_->uJBegin()-2; j--)
  {
  ...
  file << "p (" << discretization_->p().size()[0] << "x" << discretization_->p().size()[1] << "): " << std::endl 
    << std::string(fieldWidth, ' ') << "|";
  for (int i = discretization_->pIBegin()-2; i < discretization_->pIEnd()+1; i++)
  {
    file << std::setw(fieldWidth) << i;
  }
  ...
  // write p values
  for (int j = discretization_->pJEnd(); j >= discretization_->pJBegin()-2; j--)
  {
    file << std::setw(fieldWidth) << j << "|";
    for (int i = discretization_->pIBegin()-2; i < discretization_->pIEnd()+1; i++)
    {
