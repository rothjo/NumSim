...
Multigrid::Multigrid(std::shared_ptr<Discretization> baseDiscretization,
    ...
    : ParallelPressureSolver(baseDiscretization, epsilon, maximumNumberOfIterations, partitioning),
    ...
    for (const auto &grid : grids_) {
        smoothers_.emplace_back(std::make_shared<ParallelCG>(grid, epsilon, maximumNumberOfIterations, partitioning));
    }
}
